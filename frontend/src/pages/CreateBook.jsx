...
const InputField = ({ label, value, onChange, type = 'text' }) => (
  <div className='my-4'>
    ...
    <input
      ...
      className='border-2 border-gray-500 px-4 py-2 w-full' // Styling the input field with Tailwind CSS
    />
  </div>
);
...
const CreateBook = () => {
  ...
  const handleSaveBook = async () => {
    ...
    if (!title || !author || !publishYear) {
      enqueueSnackbar('All fields are required', { variant: 'warning' }); // Show warning if any field is empty
      return;
    }

    // Validation to ensure publishYear is a valid number
    if (isNaN(publishYear) || publishYear < 0) {
      enqueueSnackbar('Publish Year must be a valid positive number', { variant: 'warning' }); // Show warning for invalid publish year
      return;
    }
  ...
  return (
    <div className='p-4'>
      ...
      <div className='flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto'>
        ...
        >
          {loading ? 'Saving...' : 'Save'} {/* Show "Saving..." text when loading */}
        </button>
      </div>
    </div>
  );
